    ...
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400..900;1,400..900&display=swap" rel="stylesheet">
     <style>
        ...
        .store-header {
            ...
            margin-bottom: 2.5rem;    /* Space below the header section */
            padding-bottom: 1rem;     /* Optional: add padding below */
            border-bottom: 1px solid #eee; /* Optional: subtle line below header */
        ...
        .store-header h1 {
            font-family: "Playfair Display", serif; /* Use the site's display font */
            font-size: 2.5rem;        /* Make the title significantly larger */
            color: var(--text-dark);  /* Use the standard dark text color */
        ...
        .store-header .search-bar { /* Be specific */
        ...
        .store-header .search-bar:focus {
            ...
            border-color: var(--text-dark); /* Darken border on focus */
            box-shadow: 0 0 0 2px rgba(44, 39, 36, 0.1); /* Subtle glow */
        }
        ...
        .cart-interaction {
          margin-top: auto; /* Pushes button/selector to the bottom */
          text-align: center; /* Center button/selector */
        }
        ...
        /* --- Pagination Styles --- */
        .pagination { text-align: center; padding: 1rem 0; margin-top: 2rem; }
        .pagination a, .pagination strong, .pagination span { display: inline-block; padding: 0.6rem 1rem; margin: 0 0.25rem; border: 1px solid #ddd; text-decoration: none; color: var(--text-dark); background-color: var(--white); border-radius: 4px; font-family: "Roboto", sans-serif; font-size: 0.9rem; transition: background-color 0.3s ease, color 0.3s ease, border-color 0.3s ease; vertical-align: middle; }
        .pagination a:hover { background-color: var(--primary-color); color: var(--text-dark); border-color: #ccc; }
        .pagination strong { background-color: var(--text-dark); color: var(--primary-color); border-color: var(--text-dark); font-weight: bold; cursor: default; }
        .pagination a.prev-next-link { border: none; background-color: transparent; padding: 0.6rem 0.5rem; color: var(--text-dark); transition: color 0.3s ease; }
        .pagination a.prev-next-link:hover { color: var(--primary-color); background-color: transparent; border-color: transparent; }
        .pagination span.prev-next-disabled { border: none; background-color: transparent; padding: 0.6rem 0.5rem; cursor: default; }
        .pagination span.prev-next-disabled span { color: #aaa; padding: 0; margin: 0; border: none; background-color: transparent; }

         /* --- CSS for Quantity Selector (from style.css) --- */
        .quantity-selector { display: inline-flex; align-items: center; border: 1px solid #ccc; border-radius: 4px; margin-top: 0.5rem; }
        .quantity-selector button { background-color: #f8f8f8; border: none; cursor: pointer; font-size: 1.2rem; line-height: 1; padding: 0.5rem 0.9rem; font-weight: bold; color: #555; }
        .quantity-selector button:hover { background-color: #eee; }
        .quantity-selector .qty-display { padding: 0.5rem 1rem; font-size: 1rem; min-width: 25px; text-align: center; font-weight: bold; background-color: var(--text-dark); color: var(--primary-color); }
        .quantity-selector .minus-btn { border-radius: 4px 0 0 4px; }
        .quantity-selector .plus-btn { border-radius: 0 4px 4px 0; }
        .add-to-cart-btn { display: inline-block; padding: 0.6rem 1.2rem; background-color: var(--text-dark); color: var(--primary-color); border: none; border-radius: 4px; cursor: pointer; text-align: center; transition: background-color 0.3s ease; width: auto; margin-top: 0.5rem; }
        .add-to-cart-btn:hover { background-color: #555; }
        .cart-interaction.processing * { opacity: 0.5; pointer-events: none; }
...
<body>
    ...
    <div class="store-container">
        ...
        <div id="cart-status"></div> <div class="product-grid" id="product-grid">
            <?php
