...
// Database connection
$servername = "localhost"; $username = "root"; $password = "vyom0403"; $dbname = "jewellery_store"; // Ensure password is correct
$conn = new mysqli($servername, $username, $password, $dbname);
if ($conn->connect_error) { die("Connection failed: " . $conn->connect_error); }

// Fetch order details AND verify it belongs to the logged-in user
$stmt = $conn->prepare("SELECT * FROM orders WHERE order_id = ? AND user_id = ?");
 if (!$stmt) { die("Prepare failed: (" . $conn->errno . ") " . $conn->error); } // Check prepare success
    ...
    <link rel="stylesheet" href="./resources/css/style.css">
     ...
     <style>
        ...
        .confirmation-container {
         ...
         .order-summary p { margin-bottom: 0.8rem; line-height: 1.5; color: #555; }
         .order-summary p strong { color: #333; }

         .order-summary h3 { /* Style for "Items Ordered" */
             font-size: 1.2rem;
             margin-top: 2rem;
             margin-bottom: 1rem;
             color: #333;
         }
         .order-items-table { width: 100%; margin-top: 1rem; border-collapse: collapse; font-size: 0.95rem; }
         .order-items-table th, .order-items-table td { border: 1px solid #eee; padding: 0.8rem 1rem; text-align: left; vertical-align: middle; }
         .order-items-table th { background-color: #f8f8f8; font-weight: 600;}
         .order-items-table img { max-width: 50px; vertical-align: middle; margin-right: 10px; border-radius: 4px;}
         .order-items-table td:last-child { font-weight: 500; } /* Subtotal column */
...
<body>
    ...
    <div class="confirmation-container">
        ...
        <div class="order-summary">
            ...
            <p><strong>Order Status:</strong> <?php echo htmlspecialchars(ucfirst($order['status'])); // Capitalize status ?></p>
